-- =====================================================
-- Migration 113: Fix lead_status column name in RPC functions
-- =====================================================
-- Issue: Migration 112 referenced l_upd.lead_status but the column is named 'status'
-- The leads table has: status lead_status DEFAULT 'New'
-- So the column is 'status' of TYPE 'lead_status', not a column named 'lead_status'
-- =====================================================
...
BEGIN
    ...
    -- If still no opportunity and autocreate is not allowed, that's ok for this flow
    -- We'll just update the quotation without pipeline sync
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        ...
        ELSE
            ...
            -- UPDATED LOGIC for Migration 112:
            ...
            IF v_old_opp_stage IN ('Prospecting', 'Discovery') AND v_quotation_sequence = 1 THEN
                ...
                INSERT INTO public.pipeline_updates (
                    opportunity_id, update_type, old_value, new_value, updated_by, notes
                ) VALUES (
                    v_effective_opportunity_id,
                    'stage_change',
                    jsonb_build_object('stage', v_old_opp_stage, 'quotation_status', 'draft'),
                    jsonb_build_object('stage', v_new_opp_stage, 'quotation_status', 'sent',
                        'quotation_id', p_quotation_id, 'quotation_number', v_quotation.quotation_number,
                        'sequence', v_quotation_sequence, 'sequence_label', v_sequence_label),
                    p_actor_user_id,
                    v_sequence_label || ' quotation sent - ' || v_quotation.quotation_number
                );
                v_pipeline_updates_inserted := TRUE;

            ELSIF v_old_opp_stage = 'Quote Sent' AND v_quotation_sequence = 1 THEN
                -- Already in Quote Sent (first quotation) - just update activity, no stage change
                v_new_opp_stage := 'Quote Sent';
                v_activity_subject := v_sequence_label || ' Quotation Sent (already in Quote Sent stage)';

                -- No stage change needed, but record activity
                INSERT INTO public.pipeline_updates (
                    opportunity_id, update_type, old_value, new_value, updated_by, notes
                ) VALUES (
                    v_effective_opportunity_id,
                    'quotation_sent',
                    jsonb_build_object('quotation_status', 'draft'),
                    jsonb_build_object('quotation_status', 'sent',
                        'quotation_id', p_quotation_id, 'quotation_number', v_quotation.quotation_number,
                        'sequence', v_quotation_sequence, 'sequence_label', v_sequence_label),
                    p_actor_user_id,
                    v_sequence_label || ' quotation sent - ' || v_quotation.quotation_number
            ...
            ELSIF v_old_opp_stage = 'Negotiation' THEN
                ...
                -- No stage change, but record pipeline update
                INSERT INTO public.pipeline_updates (
                    opportunity_id, update_type, old_value, new_value, updated_by, notes
                ) VALUES (
                    v_effective_opportunity_id,
                    'quotation_sent',
                    jsonb_build_object('stage', 'Negotiation', 'quotation_status', 'draft'),
                    jsonb_build_object('stage', 'Negotiation', 'quotation_status', 'sent',
                        'quotation_id', p_quotation_id, 'quotation_number', v_quotation.quotation_number,
                        'sequence', v_quotation_sequence, 'sequence_label', v_sequence_label,
                        'previous_rejections', v_previous_rejected_count),
                    p_actor_user_id,
                    v_sequence_label || ' quotation sent during negotiation - ' || v_quotation.quotation_number
                );
                v_pipeline_updates_inserted := TRUE;
    ...
    IF v_quotation.ticket_id IS NOT NULL THEN
        ...
        BEGIN
            ...
            IF v_ticket IS NOT NULL AND v_effective_opportunity_id IS NOT NULL THEN
                INSERT INTO public.pipeline_updates (
                    opportunity_id, update_type, old_value, new_value, updated_by, notes
                ) VALUES (
                    v_effective_opportunity_id,
                    ...
                    p_actor_user_id,
                    'Ticket status updated after quotation sent'
                );
            END IF;
        END;
    END IF;
    ...
    -- FIXED: Column is 'status' not 'lead_status'
...
'Atomically marks quotation as sent with corrected lead status column reference.

MIGRATION 113 FIX:
- Fixed l_upd.lead_status to l_upd.status (correct column name)
- The leads table has column "status" of TYPE "lead_status"';
