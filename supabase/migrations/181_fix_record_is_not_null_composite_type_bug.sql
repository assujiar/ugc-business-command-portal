-- =====================================================
-- Migration 181: Fix RECORD IS NOT NULL composite type bug
-- =====================================================
--
-- ROOT CAUSE:
--   In PostgreSQL, for composite types (RECORD):
--     v_record IS NULL     → TRUE only if ALL fields are NULL
--     v_record IS NOT NULL → TRUE only if ALL fields are NOT NULL
--
--   When a row has SOME nullable columns (competitor, notes, etc.),
--   BOTH expressions can be FALSE simultaneously!
--
--   Example: opportunities row has opportunity_id='OPP123' (non-null)
--   but competitor=NULL, notes=NULL, etc.
--     v_opportunity IS NULL     → FALSE (has non-null fields)
--     v_opportunity IS NOT NULL → FALSE (has null fields!)  ← BUG
--
--   This caused ALL opportunity update, ticket update, stage history,
--   pipeline_updates, and activities to be SILENTLY SKIPPED.
--
-- FIX:
--   Replace `v_record IS NOT NULL` with `v_record.primary_key IS NOT NULL`
--   which correctly checks if the record was populated from a SELECT.
--
-- AFFECTED FUNCTIONS:
--   - rpc_customer_quotation_mark_rejected (3 occurrences)
--   - rpc_customer_quotation_mark_sent (4 occurrences)
--   - rpc_customer_quotation_mark_accepted (3 occurrences)
--
-- NOT AFFECTED:
--   - rpc_ticket_mark_won / rpc_ticket_mark_lost (use scalar SELECTs)
-- =====================================================


-- =====================================================
-- PART 1: Fix mark_rejected
-- =====================================================
DO $$
DECLARE
    v_proc RECORD;
...
BEGIN
    ...
    -- FIX (migration 181): Use .opportunity_id IS NOT NULL instead of record IS NOT NULL
    ...
    -- FIX (migration 181): Use .opportunity_id IS NOT NULL
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[181][%] mark_rejected FAILED: % (%)', v_correlation_id, SQLERRM, SQLSTATE;
...
BEGIN
    ...
    IF v_quotation.status = 'sent' THEN
        ...
        RETURN jsonb_build_object('success', TRUE, 'quotation_id', v_quotation.id, 'quotation_number', v_quotation.quotation_number,
            ...
            'probability_synced', v_opportunity.opportunity_id IS NOT NULL AND v_opportunity.probability IS DISTINCT FROM v_stage_prob,
            'correlation_id', v_correlation_id);
    END IF;
    ...
    IF v_effective_opportunity_id IS NULL AND p_allow_autocreate THEN
        BEGIN
            ...
            IF v_derived_opportunity_id IS NOT NULL THEN v_effective_opportunity_id := v_derived_opportunity_id; END IF;
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '[181][%] fn_resolve_or_create_opportunity failed: %', v_correlation_id, SQLERRM;
        END;
    END IF;
    IF v_effective_opportunity_id IS NULL AND v_quotation.lead_id IS NOT NULL THEN
        SELECT ld.opportunity_id INTO v_derived_opportunity_id FROM public.leads ld WHERE ld.lead_id = v_quotation.lead_id AND ld.opportunity_id IS NOT NULL;
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[181][%] mark_sent FAILED: % (%)', v_correlation_id, SQLERRM, SQLSTATE;
    RETURN jsonb_build_object('success', FALSE, 'error', SQLERRM, 'error_code', 'INTERNAL_ERROR', 'detail', SQLSTATE, 'correlation_id', v_correlation_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, pg_temp;
...
BEGIN
    ...
    IF v_effective_opportunity_id IS NOT NULL AND v_opportunity.opportunity_id IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '[181][%] sync_opportunity_to_account failed: %', v_correlation_id, SQLERRM;
        END;
    END IF;
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[181][%] mark_accepted FAILED: % (%)', v_correlation_id, SQLERRM, SQLSTATE;
    RETURN jsonb_build_object('success', FALSE, 'error', SQLERRM, 'error_code', 'INTERNAL_ERROR', 'detail', SQLSTATE, 'correlation_id', v_correlation_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, pg_temp;
...
-- SUMMARY
-- =====================================================
-- Fixed PostgreSQL composite type IS NOT NULL gotcha in 3 RPC functions.
--
-- In PostgreSQL, for RECORD types:
--   v_record IS NULL     → TRUE only if ALL fields are NULL (works for "not found")
--   v_record IS NOT NULL → TRUE only if ALL fields are NOT NULL (BROKEN for rows with nullable columns!)
--
-- When opportunities/tickets rows have NULL columns (competitor, notes, etc.),
-- `v_record IS NOT NULL` evaluates to FALSE even though the record was populated.
-- This silently skips ALL downstream operations (opportunity update, ticket update,
-- stage history, pipeline_updates, activities).
--
-- Fix: Replace `v_record IS NOT NULL` with `v_record.primary_key IS NOT NULL`
--
-- Changes per function:
--   mark_rejected: 4 fixes (opportunity_id x2, ticket.id x2)
--   mark_sent:     4 fixes (opportunity_id x3, ticket.id x1)
--   mark_accepted: 3 fixes (opportunity_id x2, ticket.id x1)
-- =====================================================
