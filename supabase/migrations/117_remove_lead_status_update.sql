-- =====================================================
-- Migration 117: Remove lead status update from quotation send
-- =====================================================
-- Business Logic Clarification:
-- 1. Lead → Account (1:1) - one lead becomes one account
-- 2. Account can have many: opportunities/pipelines, quotations, tickets
-- 3. Lead status should NOT be affected by quotation/ticket/pipeline updates
-- 4. Lead becomes "active" when it has a pipeline/opportunity
-- 5. Lead becomes "inactive" after 3 months without new activity
--
-- Therefore: Sending a quotation should NOT update lead status
-- =====================================================
...
BEGIN
    ...
    -- Lead status is managed separately based on account activity over time
    -- (active/inactive after 3 months without new pipeline or invoice)
...
'Atomically marks quotation as sent and syncs with opportunity/pipeline.

Business Logic:
- Updates quotation status to sent
- Updates opportunity stage based on quotation sequence:
  - 1st quotation from Prospecting/Discovery → Quote Sent
  - Quotations during Negotiation → Stay in Negotiation
- Creates pipeline_updates and activities records
- Updates ticket status to waiting_customer
