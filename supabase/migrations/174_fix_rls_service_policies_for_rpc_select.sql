-- =====================================================
-- Migration 174: Fix RLS Service Policies for RPC SELECT
-- =====================================================
-- ROOT CAUSE: SECURITY DEFINER functions run as function owner, but
-- auth.uid() is NULL (service_role JWT has no 'sub' claim).
-- Migration 099 added opp_update_service for UPDATE (auth.uid() IS NULL),
-- but there is NO corresponding SELECT service policy on opportunities
-- and NO service policies at all on tickets.
-- This blocks SELECT INTO inside RPCs, causing opportunity/ticket updates to be SKIPPED.
--
-- FIX: Add service SELECT/UPDATE policies matching the existing opp_update_service pattern.
-- Also drop all function overloads and recreate cleanly with NOTIFY pgrst.
-- =====================================================

-- PART 0: Diagnostic - log the current user and superuser status
DO $$
DECLARE
    v_is_super BOOLEAN;
    v_current_user TEXT;
BEGIN
    ...
    SELECT usesuper INTO v_is_super FROM pg_user WHERE usename = v_current_user;
    RAISE WARNING '[174] Migration running as user=%, is_superuser=%', v_current_user, v_is_super;
END $$;

-- =====================================================
-- PART 1: Add missing RLS service policies
-- =====================================================

-- 1a. opportunities SELECT service policy (mirrors opp_update_service from migration 099)
...
-- 1d. tickets INSERT service policy (for ticket_events, ticket_comments created by RPCs)
...
-- 1e. ticket_events/ticket_comments SELECT service policy
...
-- 1f. pipeline_updates INSERT/SELECT service policy
...
-- 1g. activities INSERT/SELECT service policy
...
-- 1h. opportunity_stage_history INSERT service policy
...
-- 1i. opportunities UPDATE service policy (already exists from 099 but re-ensure)
...
-- 1j. customer_quotations service policies
...
-- 1k. leads UPDATE service policy
...
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'leads') THEN
        IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'leads' AND n.nspname = 'public' AND c.relrowsecurity = true) THEN
...
-- 1l. quotation_rejection_reasons service policy
DROP POLICY IF EXISTS qrr_insert_service ON public.quotation_rejection_reasons;
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'quotation_rejection_reasons' AND n.nspname = 'public' AND c.relrowsecurity = true) THEN
...
-- 1m. ticket_rate_quotes (operational costs) service policies
...
DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE c.relname = 'ticket_rate_quotes' AND n.nspname = 'public' AND c.relrowsecurity = true) THEN
...
-- =====================================================
-- PART 2: Drop ALL overloads of all 3 RPCs
-- =====================================================
DO $$
DECLARE
    v_proc RECORD;
BEGIN
    -- Drop all overloads of mark_rejected
    FOR v_proc IN
    ...
    LOOP
        ...
        RAISE WARNING '[174] Dropped function: %', v_proc.proc_sig;
    END LOOP;

    -- Drop all overloads of mark_sent
    FOR v_proc IN
        SELECT p.oid::regprocedure AS proc_sig
        FROM pg_proc p
        JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    -- Drop all overloads of mark_accepted
    FOR v_proc IN
        SELECT p.oid::regprocedure AS proc_sig
        FROM pg_proc p
        JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    RAISE WARNING '[174] All RPC overloads dropped successfully';
END $$;

-- =====================================================
-- PART 3: Recreate fn_stage_config
-- =====================================================
CREATE OR REPLACE FUNCTION public.fn_stage_config(p_stage opportunity_stage)
RETURNS TABLE(probability INTEGER, next_step TEXT, days_allowed INTEGER) AS $fn$
BEGIN
    RETURN QUERY SELECT
        CASE p_stage
        ...
        END,
        CASE p_stage
            WHEN 'Prospecting' THEN 'Identify Decision Maker'::TEXT
            WHEN 'Discovery' THEN 'Submit Quotation'::TEXT
            WHEN 'Quote Sent' THEN 'Follow Up Customer'::TEXT
            WHEN 'Negotiation' THEN 'Close Deal'::TEXT
            WHEN 'Closed Won' THEN NULL::TEXT
            WHEN 'Closed Lost' THEN NULL::TEXT
            ELSE NULL::TEXT
        END,
        CASE p_stage
            WHEN 'Prospecting' THEN 7
            WHEN 'Discovery' THEN 14
...
-- =====================================================
-- PART 4: Recreate rpc_customer_quotation_mark_rejected
-- =====================================================
...
DECLARE
    ...
    v_debug_opp_found BOOLEAN := FALSE;
    v_debug_ticket_found BOOLEAN := FALSE;
BEGIN
    v_correlation_id := COALESCE(p_correlation_id, gen_random_uuid()::TEXT);
    v_actor_id := COALESCE(p_actor_user_id, auth.uid());

    RAISE WARNING '[174][%] mark_rejected START qid=% current_user=% session_user=% auth.uid=%',
        v_correlation_id, p_quotation_id, current_user, session_user, auth.uid();
    ...
    RAISE WARNING '[174][%] quotation found: id=%, ticket_id=%, opp_id=%, lead_id=%',
        v_correlation_id, v_quotation.id, v_saved_ticket_id, v_quotation.opportunity_id, v_quotation.lead_id;
    ...
    RAISE WARNING '[174][%] derived: opp_id=%, lead_id=%', v_correlation_id, v_effective_opportunity_id, v_effective_lead_id;
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        ...
        v_debug_opp_found := FOUND;
        RAISE WARNING '[174][%] opportunity SELECT: FOUND=%, v_opportunity IS NULL=%',
            v_correlation_id, FOUND, (v_opportunity IS NULL);
    ...
    IF v_saved_ticket_id IS NOT NULL THEN
        ...
        v_debug_ticket_found := FOUND;
        RAISE WARNING '[174][%] ticket SELECT: FOUND=%, v_ticket IS NULL=%',
            v_correlation_id, FOUND, (v_ticket IS NULL);
    ...
    RAISE WARNING '[174][%] mark_rejected COMPLETE: opp_found=%, ticket_found=%, pipeline_updated=%, activities=%',
        v_correlation_id, v_debug_opp_found, v_debug_ticket_found, v_pipeline_updated, v_activities_inserted;
    ...
    RETURN jsonb_build_object(
        ...
        'correlation_id', v_correlation_id,
        '_debug_opp_found', v_debug_opp_found, '_debug_ticket_found', v_debug_ticket_found,
        '_debug_current_user', current_user, '_debug_session_user', session_user
    );
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[174][%] mark_rejected FAILED: % (%) current_user=% session_user=%',
        v_correlation_id, SQLERRM, SQLSTATE, current_user, session_user;
...
-- =====================================================
-- PART 5: Recreate rpc_customer_quotation_mark_sent
-- (Same structure as migration 173, with RLS policies now in place)
-- =====================================================
...
BEGIN
    ...
    RAISE WARNING '[174][%] mark_sent START qid=% current_user=%', v_correlation_id, p_quotation_id, current_user;
    ...
    IF v_effective_opportunity_id IS NULL THEN
        BEGIN
            ...
            IF v_derived_opportunity_id IS NOT NULL THEN v_effective_opportunity_id := v_derived_opportunity_id; END IF;
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '[174][%] fn_resolve_or_create_opportunity failed: %', v_correlation_id, SQLERRM;
        END;
    END IF;
    IF v_effective_opportunity_id IS NULL AND v_quotation.lead_id IS NOT NULL THEN
        SELECT ld.opportunity_id INTO v_derived_opportunity_id FROM public.leads ld WHERE ld.lead_id = v_quotation.lead_id AND ld.opportunity_id IS NOT NULL;
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        SELECT opp.* INTO v_opportunity FROM public.opportunities opp WHERE opp.opportunity_id = v_effective_opportunity_id FOR UPDATE;
        RAISE WARNING '[174][%] mark_sent opp SELECT: FOUND=%, opp_id=%', v_correlation_id, FOUND, v_effective_opportunity_id;
        IF v_opportunity IS NOT NULL THEN
            v_old_opp_stage := v_opportunity.stage;
            IF v_effective_lead_id IS NULL AND v_opportunity.source_lead_id IS NOT NULL THEN
                v_effective_lead_id := v_opportunity.source_lead_id;
    ...
    IF v_saved_ticket_id IS NOT NULL THEN
        SELECT t.* INTO v_ticket FROM public.tickets t WHERE t.id = v_saved_ticket_id FOR UPDATE;
        RAISE WARNING '[174][%] mark_sent ticket SELECT: FOUND=%', v_correlation_id, FOUND;
        IF v_ticket IS NOT NULL AND v_ticket.status NOT IN ('closed', 'resolved') THEN
            UPDATE public.tickets t_upd
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[174][%] mark_sent FAILED: % (%)', v_correlation_id, SQLERRM, SQLSTATE;
    RETURN jsonb_build_object('success', FALSE, 'error', SQLERRM, 'error_code', 'INTERNAL_ERROR', 'detail', SQLSTATE, 'correlation_id', v_correlation_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, pg_temp;
...
-- =====================================================
-- PART 6: Recreate rpc_customer_quotation_mark_accepted
-- =====================================================
...
BEGIN
    ...
    RAISE WARNING '[174][%] mark_accepted START qid=%', v_correlation_id, p_quotation_id;
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        SELECT opp.* INTO v_opportunity FROM public.opportunities opp WHERE opp.opportunity_id = v_effective_opportunity_id FOR UPDATE;
        RAISE WARNING '[174][%] mark_accepted opp SELECT: FOUND=%', v_correlation_id, FOUND;
        IF v_opportunity IS NOT NULL THEN
            v_old_opp_stage := v_opportunity.stage;
            IF v_effective_lead_id IS NULL AND v_opportunity.source_lead_id IS NOT NULL THEN
                v_effective_lead_id := v_opportunity.source_lead_id;
    ...
    IF v_effective_opportunity_id IS NOT NULL AND v_opportunity IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '[174][%] sync_opportunity_to_account failed: %', v_correlation_id, SQLERRM;
        END;
    END IF;
...
EXCEPTION WHEN OTHERS THEN
    RAISE WARNING '[174][%] mark_accepted FAILED: % (%)', v_correlation_id, SQLERRM, SQLSTATE;
    RETURN jsonb_build_object('success', FALSE, 'error', SQLERRM, 'error_code', 'INTERNAL_ERROR', 'detail', SQLSTATE, 'correlation_id', v_correlation_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, pg_temp;
...
GRANT EXECUTE ON FUNCTION public.rpc_customer_quotation_mark_sent(UUID, TEXT, TEXT, UUID, TEXT) TO service_role;
