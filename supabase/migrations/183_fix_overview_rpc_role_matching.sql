-- Migration 183: Fix overview RPC role matching and missing TOTAL response metrics
--
-- Problems:
-- 1. Role casing mismatch: RPC checks 'super_admin' but DB has 'super admin',
--    RPC checks 'director' but DB has 'Director', etc. ALL users fall to scope='user'.
-- 2. TOTAL response_time_metrics section missing avg_response and first_quote.
-- 3. Quotation/cost scope='user' only checks created_by, not ticket assignment.
--    Ops user can't see quotations for their assigned tickets.
...
BEGIN
    ...
    -- FIX (migration 183): Case-insensitive role matching with actual DB values
    IF LOWER(p_role) IN ('super admin', 'super_admin', 'director', 'ticketing director', 'ticketing_director') THEN
        v_scope := 'all';
    ELSIF LOWER(p_role) LIKE '%manager%' THEN
        v_scope := 'department';
    ...
    SELECT jsonb_build_object(
        ...
        -- FIX (migration 183): TOTAL now includes avg_response and first_quote
        'TOTAL', jsonb_build_object(
            'first_response', jsonb_build_object(
    ...
    -- FIX (migration 183): scope='user' also checks ticket assignment
    -- ============================================
