-- =====================================================
-- Migration 188: Fix ENUM type cast + role mapping gaps
-- =====================================================
--
-- Fixes from verification audit:
--
-- 1. CRITICAL: v_user_department was TEXT, compared against t.department
--    (ticketing_department ENUM). PostgreSQL silently casts enum→text,
--    LOSING index usage on idx_tickets_department. Fix: declare as
--    ticketing_department ENUM type so comparison uses native operator.
--
-- 2. CRITICAL: 'traffic & warehous' role did NOT match any pattern
--    (%ops%, %manager%, macx, sales support) → fell to scope='user'.
--    Fix: add LIKE '%traffic%' and LIKE '%warehous%' to scope check.
--
-- 3. HIGH: v_is_ops missed 'traffic & warehous' → quotation_analytics
--    was NOT skipped for this ops role. Fix: include traffic/warehouse.
--
-- 4. MEDIUM: ELSE fallback used UPPER(LEFT(p_department, 3)) which
--    produces 'MAR', 'OPE' etc — NOT valid ticketing_department values.
--    Would crash if variable is ENUM. Fix: proper profiles.department
--    → ticketing_department mapping with NULL fallback.
--
-- 5. MEDIUM: Marcomm, DGO, VDCO (all Marketing dept in profiles)
--    fell to scope='user'. MACX (also Marketing) was already scope=
--    'department'. Fix: add these to scope='department' for MKT.
--
-- =====================================================

-- Drop existing 5-param signature from migration 187
...
DECLARE
    ...
    v_user_id UUID;
    v_user_department ticketing_department := NULL;  -- FIX #1: was TEXT, now proper ENUM
...
BEGIN
    ...
    -- ========================================
    -- SCOPE LOGIC (fixed role mapping)
    -- ========================================
    ...
    ELSIF LOWER(p_role) LIKE '%ops%'
          ...
          OR LOWER(p_role) LIKE '%traffic%'      -- FIX #2: traffic & warehous
          OR LOWER(p_role) LIKE '%warehous%'      -- FIX #2: traffic & warehous
          OR LOWER(p_role) IN ('macx', 'sales support', 'marcomm', 'dgo', 'vdco')  -- FIX #5
    THEN
        v_scope := 'department';

        -- Map role → ticketing_department ENUM (valid values: MKT, SAL, DOM, EXI, DTD, TRF)
        v_user_department := CASE
            ...
            -- FIX #4: fallback maps profiles.department → ticketing_department
        ...
        -- FIX #3: v_is_ops should include traffic & warehouse
        v_is_ops := LOWER(p_role) LIKE '%ops%'
                    OR LOWER(p_role) LIKE '%traffic%'
                    OR LOWER(p_role) LIKE '%warehous%';

        -- Safety net: if we can't determine department, fall back to user scope
    ...
    pending_sla AS (
        SELECT
            ...
            COUNT(*) FILTER (WHERE t.status NOT IN ('resolved'::ticket_status, 'closed'::ticket_status)) as pending_resolution
        FROM public.tickets t
    ...
    -- ============================================
    -- SECTION 7: Leaderboards (NO view_mode filter - always aggregate)
    -- ============================================
    WITH user_stats AS (
        SELECT
            p.user_id,
