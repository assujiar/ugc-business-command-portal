-- =====================================================
-- Migration 190: Fix ops cost rejection reasons double counting
-- =====================================================
--
-- ROOT CAUSE: The cost_rejection_reasons CTE in rpc_ticketing_overview_v2
-- used UNION ALL between operational_cost_rejection_reasons and
-- quotation_rejection_reasons. Both tables store the same reason for
-- revise_requested ops costs, causing each rejection to be counted twice
-- (4 instead of 2 for 2 ops costs).
--
-- FIX: Only use operational_cost_rejection_reasons as the single source.
--
-- =====================================================
...
BEGIN
    ...
    -- FIX (migration 190): Only use operational_cost_rejection_reasons.
    -- Previous version used UNION ALL with quotation_rejection_reasons,
    -- causing double counting (both tables store the same reason for
    -- revise_requested ops costs).
    -- ============================================
    WITH ops_data AS (
        SELECT
            trq.status,
