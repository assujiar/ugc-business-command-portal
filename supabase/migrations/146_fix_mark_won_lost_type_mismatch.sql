-- =====================================================
-- Migration 146: Fix mark_won/mark_lost type mismatch
-- =====================================================
-- BUG: Both rpc_ticket_mark_won and rpc_ticket_mark_lost declare
-- v_old_stage as TEXT, but opportunity_stage_history.old_stage and
-- .new_stage columns are opportunity_stage ENUM type.
-- This causes: "column old_stage is of type opportunity_stage but
-- expression is of type text" when trying to close a ticket.
--
-- FIX: Cast the TEXT values to opportunity_stage in the INSERT.
-- =====================================================
...
BEGIN
    ...
    IF v_ticket.opportunity_id IS NOT NULL THEN
        DECLARE
            v_old_stage opportunity_stage;  -- FIX: was TEXT, now correctly typed
        BEGIN
            ...
            IF v_old_stage NOT IN ('Closed Won', 'Closed Lost') THEN
                ...
                INSERT INTO public.opportunity_stage_history (opportunity_id, old_stage, new_stage, changed_by, notes)
                VALUES (v_ticket.opportunity_id, v_old_stage, 'Closed Won'::opportunity_stage, v_user_id,
    ...
    IF v_ticket.opportunity_id IS NOT NULL THEN
        ...
        BEGIN
            ...
            IF v_old_stage NOT IN ('Closed Won', 'Closed Lost') THEN
                ...
                INSERT INTO public.opportunity_stage_history (opportunity_id, old_stage, new_stage, changed_by, notes)
                VALUES (v_ticket.opportunity_id, v_old_stage, 'Closed Lost'::opportunity_stage, v_user_id,
...
-- SUMMARY
-- ============================================
-- Fixed type mismatch in both rpc_ticket_mark_won and rpc_ticket_mark_lost:
-- - v_old_stage changed from TEXT to opportunity_stage
-- - 'Closed Won'/'Closed Lost' literals cast to ::opportunity_stage
