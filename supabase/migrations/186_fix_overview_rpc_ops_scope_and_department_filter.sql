-- =====================================================
-- Migration 186: Fix overview RPC scope for ops roles + department filter
-- =====================================================
--
-- PROBLEMS:
-- 1. Ops roles (domestics Ops, EXIM Ops, etc.) fall to scope='user' instead of 'department'.
--    They only see their own tickets, not their department's.
-- 2. Department filter uses profiles.department (e.g., 'Operations') which is wrong for ops.
--    Ops users all have profiles.department='Operations' regardless of ticket department.
-- 3. Quotation analytics still computed and returned for ops users (wasted computation).
--
-- FIX:
-- 1. Recognize ops roles + sales support + MACX as scope='department'.
-- 2. Map ALL department roles to TICKET department codes (EXI, DOM, DTD, TRF, MKT, SAL).
-- 3. Change ALL department filters from profiles.department to t.department.
-- 4. Skip quotation analytics for ops users (return empty object).
-- 5. Add is_ops to meta for frontend fallback.
-- =====================================================
...
DECLARE
    ...
    v_user_id UUID;
    v_user_department TEXT;  -- Now stores TICKET department code (EXI, DOM, MKT, SAL, etc.)
...
BEGIN
    ...
    -- FIX (migration 186): Comprehensive role → scope + ticket department mapping
    IF LOWER(p_role) IN ('super admin', 'super_admin', 'director', 'ticketing director', 'ticketing_director') THEN
        v_scope := 'all';
    ELSIF LOWER(p_role) LIKE '%ops%' OR LOWER(p_role) LIKE '%manager%'
          OR LOWER(p_role) IN ('macx', 'sales support') THEN
        v_scope := 'department';
        -- Map role to TICKET department code
        v_user_department := CASE
            WHEN LOWER(p_role) LIKE '%exim%' THEN 'EXI'
            WHEN LOWER(p_role) LIKE '%domestic%' THEN 'DOM'
            WHEN LOWER(p_role) LIKE '%dtd%' OR LOWER(p_role) LIKE '%import%dtd%' THEN 'DTD'
            WHEN LOWER(p_role) LIKE '%traffic%' OR LOWER(p_role) LIKE '%warehous%' THEN 'TRF'
            WHEN LOWER(p_role) LIKE '%marketing%' OR LOWER(p_role) = 'macx' THEN 'MKT'
            WHEN LOWER(p_role) LIKE '%sales%' THEN 'SAL'
            ELSE UPPER(LEFT(COALESCE(p_department, 'UNK'), 3))
        END;
        -- Flag ops users (for skipping quotation analytics)
    ...
    -- FIX (186): department filter uses t.department
    -- ============================================
    ...
    -- FIX (186): Skip for ops users (they don't need customer quotation data)
    -- ============================================
    ...
    ELSE
        -- Ops users: skip quotation analytics (hidden in frontend)
        v_quotation_analytics := '{}'::JSONB;
    END IF;
