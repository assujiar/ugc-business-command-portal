-- ============================================
-- Migration: 133_fix_activity_column_names.sql
--
-- PURPOSE: Fix activity INSERT statements using wrong column names
--
-- ISSUE:
-- Migration 131 used wrong column names for activities table:
-- - Used: opportunity_id, performed_by
-- - Should be: related_opportunity_id, owner_user_id, due_date, status, etc.
--
-- This caused activities to not be inserted, resulting in missing
-- timeline entries for quotations sent after migration 131.
--
-- SOLUTION:
-- Recreate all three RPC functions with correct activities column names.
-- ============================================

-- ============================================
-- 1. FIX: rpc_customer_quotation_mark_sent
-- Correct activity INSERT columns
-- ============================================
...
BEGIN
    ...
    IF v_effective_opportunity_id IS NOT NULL AND NOT v_is_resend THEN
        ...
        IF v_opportunity IS NOT NULL THEN
            ...
            -- Build activity subject and description
            ...
            -- Insert activity with CORRECT column names (FIX from migration 133)
...
FIX in migration 131: Multi-shipment cost sync
FIX in migration 133: Correct activity column names (related_opportunity_id, owner_user_id, due_date, status)';
...
-- SUMMARY
-- ============================================
-- Activity Column Names Fix:
--
-- ISSUE:
-- Migration 131 used wrong column names for activities table INSERT:
-- - Used: opportunity_id (doesn't exist)
-- - Used: performed_by (doesn't exist)
-- - Missing: due_date (required), status (required)
--
-- This caused the INSERT to silently fail (caught by EXCEPTION block),
-- resulting in missing activity timeline entries for quotation 4+
--
-- SOLUTION:
-- Use correct column names from activities table schema:
-- - related_opportunity_id (not opportunity_id)
-- - related_lead_id
-- - related_account_id
-- - owner_user_id (not performed_by)
-- - created_by
-- - activity_type (activity_type_v2 enum: 'Email')
-- - status (activity_status enum: 'Completed')
-- - due_date (required, set to CURRENT_DATE)
-- - completed_at
--
-- This fix ensures all future quotation sent/rejected/accepted activities
-- will be properly recorded in the timeline.
-- ============================================
