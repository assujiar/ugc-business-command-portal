-- ============================================
-- Migration: 138_fix_rpc_regressions_and_grants.sql
--
-- PURPOSE: Fix regressions introduced in migrations 131-136 where
-- RPC functions were recreated with incorrect column references,
-- missing account activation fields, and missing validation.
--
-- FIXES:
-- 1. mark_sent (from 134): expected_value→estimated_value, pipeline_updates
--    wrong columns, pending_response_from='customer'→'creator', wrong stage names
-- 2. mark_accepted (from 136): missing account fields (is_active, first_deal_date,
--    etc.), missing ticket close fields (close_reason, resolved_at, pending_response_from)
-- 3. mark_rejected (from 136): missing competitor/budget on opportunity update,
--    missing numeric validation
-- 4. Missing service_role GRANTs for rpc_ticket_create, rpc_ticket_transition
-- 5. ApproachMethod TypeScript sync note
-- ============================================


-- ============================================
-- PART 1: FIX mark_accepted - Add missing account + ticket fields
-- ============================================

-- Only update the account and ticket sections that are wrong.
-- We recreate the full function since PL/pgSQL requires it.
...
BEGIN
    ...
    IF v_quotation.status = 'accepted' THEN
        RETURN jsonb_build_object('success', TRUE, 'quotation_id', v_quotation.id, 'quotation_number', v_quotation.quotation_number, 'quotation_status', v_quotation.status, 'is_idempotent', TRUE, 'message', 'Already accepted.', 'correlation_id', v_correlation_id);
    END IF;
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        ...
        IF v_opportunity IS NOT NULL THEN
            ...
            SET stage = v_new_opp_stage, estimated_value = COALESCE(v_quotation.total_selling_rate, estimated_value),
            ...
            IF v_stage_changed THEN
                IF NOT EXISTS (
                    ...
                    VALUES (v_effective_opportunity_id, v_old_opp_stage, v_new_opp_stage, v_actor_id,
                        '[' || v_correlation_id || '] Deal won - quotation accepted', NOW());
            ...
            IF NOT EXISTS (
                ...
                AND pu.old_stage = v_old_opp_stage AND pu.new_stage = v_new_opp_stage
                ...
                VALUES (v_effective_opportunity_id,
                    ...
                    'Email'::approach_method, v_old_opp_stage, v_new_opp_stage, v_actor_id, NOW());
            ...
            -- FIX: Update account with ALL activation fields (regression from 078→136)
            IF v_opportunity.account_id IS NOT NULL THEN
                ...
                AND account_status IN ('calon_account', 'new_account');
            END IF;
        END IF;
    END IF;

    -- 3. UPDATE TICKET -> closed (with ALL fields)
    IF v_quotation.ticket_id IS NOT NULL THEN
        ...
        IF v_ticket IS NOT NULL THEN
            ...
            -- FIX: Include close_reason, resolved_at, pending_response_from (regression from 078→136)
            ...
            VALUES (v_quotation.ticket_id, 'customer_quotation_accepted'::ticket_event_type, v_actor_id,
    ...
    RETURN jsonb_build_object(
        ...
        'ticket_status', v_return_ticket_status, 'close_outcome', 'won',
...
-- ============================================
-- PART 2: FIX mark_rejected - Add competitor/budget + validation
-- ============================================
...
BEGIN
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        ...
        IF v_opportunity IS NOT NULL THEN
            ...
            ELSE
                -- FIX: Still update competitor/budget even if stage doesn't change
                UPDATE public.opportunities opp_upd
...
-- ============================================
-- PART 3: FIX mark_sent - Fix column references
-- Based on migration 134 with these fixes:
-- 1. 'New Lead','Qualified' → 'Prospecting','Discovery' (correct opportunity_stage values)
-- 2. expected_value → estimated_value
-- 3. pipeline_updates: update_type/old_value/new_value → approach_method/old_stage/new_stage
-- 4. pending_response_from = 'customer' → 'creator'
-- ============================================
...
BEGIN
    ...
    IF v_effective_opportunity_id IS NOT NULL AND NOT v_is_resend THEN
        ...
        IF v_opportunity IS NOT NULL THEN
            ...
            -- FIX: Correct stage names (was 'New Lead','Qualified' which are NOT opportunity_stage values)
            IF v_opportunity.stage IN ('Prospecting', 'Discovery') THEN
                v_new_opp_stage := 'Quote Sent'::opportunity_stage;
                UPDATE public.opportunities opp_upd
                SET stage = v_new_opp_stage,
                    -- FIX: estimated_value (was expected_value which doesn't exist)
            ...
            -- FIX: pipeline_updates with CORRECT columns (was update_type/old_value/new_value)
            IF NOT EXISTS (
                SELECT 1 FROM public.pipeline_updates pu
                WHERE pu.opportunity_id = v_effective_opportunity_id
    ...
    IF v_quotation.ticket_id IS NOT NULL AND NOT v_is_resend THEN
        ...
        IF v_ticket IS NOT NULL THEN
            ...
            SET status = 'waiting_customer'::ticket_status,
                -- FIX: 'creator' not 'customer' (response_owner enum only has creator/assignee)
    ...
    RETURN jsonb_build_object(
        ...
        'sequence_label', v_sequence_label, 'multi_shipment_costs_updated', v_multi_cost_count,
        'correlation_id', v_correlation_id
    );
EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('success', FALSE, 'error', SQLERRM, 'error_code', 'INTERNAL_ERROR', 'detail', SQLSTATE, 'correlation_id', v_correlation_id);
...
-- ============================================
-- PART 4: SERVICE_ROLE GRANTS
-- ============================================

-- Missing grants for ticket RPCs (used by adminClient in API routes)
GRANT EXECUTE ON FUNCTION public.rpc_ticket_create TO service_role;
GRANT EXECUTE ON FUNCTION public.rpc_ticket_transition TO service_role;

-- Re-grant updated functions to both authenticated and service_role
...
-- ============================================
-- SUMMARY OF FIXES IN MIGRATION 138
-- ============================================
-- 1. mark_accepted: Restored account activation fields (is_active, first_deal_date,
--    first_transaction_date, last_transaction_date). Restored ticket close fields
--    (close_reason, resolved_at, pending_response_from=NULL). Fixed event type to
--    customer_quotation_accepted.
-- 2. mark_rejected: Restored numeric validation for competitor/budget rejection reasons.
--    Restored competitor/budget fields update on opportunity.
-- 3. mark_sent: Fixed expected_value→estimated_value, 'New Lead'/'Qualified'→
--    'Prospecting'/'Discovery', pipeline_updates wrong columns→correct columns,
--    pending_response_from='customer'→'creator', event type to customer_quotation_sent.
-- 4. Added service_role GRANTs for rpc_ticket_create and rpc_ticket_transition.
-- ============================================
