-- =====================================================
-- Migration 116: Fix opportunity_stage_history column name
-- =====================================================
-- Issue: Migration 115 used wrong column name:
--   - change_reason (doesn't exist, should be 'reason')
--
-- Correct columns in opportunity_stage_history table:
...
--   - changed_by
--   - changed_at
--   - reason (NOT change_reason)
--   - notes
--   - old_stage (added in migration 023)
--   - new_stage (added in migration 023)
-- =====================================================
    ...
    IF v_effective_opportunity_id IS NOT NULL THEN
        ...
        ELSE
            ...
            IF v_old_opp_stage IN ('Prospecting', 'Discovery') AND v_quotation_sequence = 1 THEN
                ...
                -- Insert stage history with CORRECT column name 'reason' (not 'change_reason')
...
'Atomically marks quotation as sent with all correct column names.

MIGRATION 116 FIXES:
- opportunity_stage_history: use "reason" not "change_reason"
- activities: use related_opportunity_id, related_lead_id, related_account_id, owner_user_id
- pipeline_updates: use approach_method, old_stage, new_stage, notes
- leads: use "status" not "lead_status"
