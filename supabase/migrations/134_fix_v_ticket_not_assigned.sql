-- ============================================
-- Migration: 134_fix_v_ticket_not_assigned.sql
--
-- PURPOSE: Fix "v_ticket is not assigned yet" error
--
-- ISSUE:
-- Migration 133 has a bug where v_ticket.status is used in the return
-- statement, but v_ticket is only assigned when:
-- 1. quotation has a ticket_id, AND
-- 2. it's NOT a resend
--
-- For resend operations or quotations without tickets, v_ticket is never
-- assigned, causing the error.
--
-- SOLUTION:
-- Use COALESCE to handle NULL v_ticket safely in return statements.
-- ============================================
...
DECLARE
    ...
    -- Ticket status for return (safe from NULL)
...
BEGIN
    ...
    IF v_quotation.ticket_id IS NOT NULL AND NOT v_is_resend THEN
        ...
        IF v_ticket IS NOT NULL THEN
            ...
            -- Store ticket status for return
    ...
    -- 5. UPDATE OPERATIONAL COST (if linked) - SINGLE COST
    ...
    -- 5b. UPDATE ALL OPERATIONAL COSTS in operational_cost_ids array (MULTI-SHIPMENT)
    ...
    -- FIX: Use v_return_ticket_status instead of v_ticket.status to avoid NULL reference
    RETURN jsonb_build_object(
        'success', TRUE,
        'quotation_id', v_quotation.id,
        'quotation_number', v_quotation.quotation_number,
...
BEGIN
    ...
    -- FIX: Use v_return_ticket_status instead of v_ticket.status
    RETURN jsonb_build_object(
        'success', TRUE,
        'quotation_id', v_quotation.id,
        'quotation_number', v_quotation.quotation_number,
...
-- SUMMARY
-- ============================================
-- Fix: "v_ticket is not assigned yet" error
--
-- ISSUE:
-- In migration 133, the return statement used v_ticket.status directly,
-- but v_ticket is only assigned when:
-- 1. quotation has a ticket_id, AND
-- 2. it's NOT a resend (for mark_sent)
--
-- SOLUTION:
-- Added v_return_ticket_status variable that is safely assigned only
-- when v_ticket is actually populated. The return statement now uses
-- this safe variable instead of v_ticket.status directly.
-- ============================================
