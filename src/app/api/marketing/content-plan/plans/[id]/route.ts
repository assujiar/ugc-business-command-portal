...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    let children: any[] = []
    if (plan) {
      const { data: childPlans } = await (admin as any)
        .from('marketing_content_plans')
    ...
    const { data: activity } = await (admin as any)
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { data: plans, error } = await (admin as any)
      .from('marketing_content_plans')
      .update(updateFields)
      .eq('id', id)
      .select()
    ...
    if (hashtag_ids !== undefined) {
      await (admin as any).from('marketing_content_plan_hashtags').delete().eq('content_plan_id', id)
      if (hashtag_ids.length > 0) {
        const links = hashtag_ids.map((hid: number) => ({ content_plan_id: id, hashtag_id: hid }))
        await (admin as any).from('marketing_content_plan_hashtags').insert(links)
      }
    }
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { error } = await (admin as any).from('marketing_content_plans').delete().eq('id', id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
