...
import { runDailySEOFetch, runWeeklyVitalsFetch, fetchPageSpeedData, fetchGSCData, fetchGA4Data, fetchGoogleAdsData, fetchGA4Demographics, fetchGA4UTMData } from '@/lib/seo-sem-fetcher'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'manual') {
      ...
      if (specificService === 'google_search_console') {
      ...
      } else if (specificService === 'ga4_utm') {
        const endDt = target_date || new Date(Date.now() - 1 * 86400000).toISOString().split('T')[0]
        results.push({ service: 'ga4_utm', ...await fetchGA4UTMData(startDt, endDt) })
      } else {
        ...
        results.push({ service: 'ga4_utm', ...await fetchGA4UTMData(startDt, adsEnd) })
      }
    }
