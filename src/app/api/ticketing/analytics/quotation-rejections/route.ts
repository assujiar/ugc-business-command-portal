...
// GET /api/ticketing/analytics/quotation-rejections - Get quotation rejection analytics
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await (supabase as any)
      .from('vw_quotation_rejection_analytics')
      .select('*')
      .order('month', { ascending: false })

    if (error) {
      console.error('Error fetching quotation rejection analytics:', error)
